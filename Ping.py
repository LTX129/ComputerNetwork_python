...
def doOnePing(destinationAddress, timeout, dataSize):
    ...
    timeSent = sendOnePing(icmpSocket, destinationAddress, ID, dataSize)

    # Call the receiveOnePing function
    delay, ipTTL, dataSize = receiveOnePing(icmpSocket, destinationAddress, ID, timeout, timeSent, dataSize)

    # Close the ICMP socket
    icmpSocket.close()

    # Return the total network delay, TTL, and data size
    return delay, ipTTL, dataSize
